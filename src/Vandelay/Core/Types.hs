...
texify or (ValData v s) = surroundText or (changeAllZeros (commaPrintf (or^.oFormat) (or^.oScale * v))) ++ makeStars s

makeStars ∷ Int → Text
makeStars i | i == 0    = ""
            | otherwise = "\\sym{" ++ replicate i '*' ++ "}"

surroundText ∷ OutputRequest → Text → Text
surroundText or s = let (prefix, postfix) = or^.oSurround
                    in  prefix ++ s ++ postfix

changeAllZeros ∷ Text → Text
