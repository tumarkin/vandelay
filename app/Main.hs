...
run cmd = runSimpleApp . runExceptT $ 
    case cmd of
...
printError (Left err) =
    RT.putChunkLn (Rainbow.chunk "Vandelay error:" & fore red)
...
parseInit =
    Init <$> some (argument str (metavar "TAB-SEPARATED-FILE(S)"))
         <*> parseOutput

parseMake ∷ Parser Command
parseMake =
    Make <$> some (argument str (metavar "VANDELAY-TEMPLATE(S)"))
         <*> outputPath
  where
...
parseDhall =
    Dhall <$> argument str (metavar "DHALL-LIBRARY-PATH")

-- Initialization options

parseOutput ∷ Parser Output
parseOutput = optional $ 
    strOption
        ( long "output"
            <> short 'o'
            <> metavar "FILENAME"
